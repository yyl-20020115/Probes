 ...
 * IIC_write(unsigned char slave_addr, unsigned char reg_addr,
...
#include "msp430_interrupt.h"
#include "log.h"
#define IIC_write   msp430_IIC_write
...
#define IIC_write(a, b, c, d)   twi_write(a, b, d, c)
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (IIC_write(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_init(void)
{
    ...
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
        return -1;
    delay_ms(100);
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (IIC_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (rate == 1)
        ...
        tmp[0] = INV_LPA_640HZ;
    if (IIC_write(st.hw->addr, st.reg->lp_accel_odr, 1, tmp))
        return -1;
    tmp[0] = BIT_LPA_CYCLE;
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (IIC_write(st.hw->addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (IIC_write(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (IIC_write(st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (IIC_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (IIC_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (IIC_write(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (IIC_write(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (IIC_write(st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (IIC_write(st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (IIC_write(st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (IIC_write(st.hw->addr, st.reg->s4_ctrl, 1, &div))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data)) {
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (IIC_write(st.hw->addr, st.reg->s1_do, 1, &data))
        return -1;
    /* Enable/disable I2C master mode. */
    if (IIC_write(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
#endif
#endif
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (IIC_write(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (IIC_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int compass_self_test(void)
{
    ...
    if (IIC_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp))
        return 0x07;
    tmp[0] = AKM_BIT_SELF_TEST;
    if (IIC_write(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp))
        goto AKM_restore;
...
AKM_restore:
    ...
    IIC_write(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp);
    tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
    IIC_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp);
    mpu_set_bypass(0);
    return result;
}
#endif
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (IIC_write(st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (IIC_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (IIC_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (IIC_write(st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
        ...
        unsigned char *data)
{
    ...
    if (IIC_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (IIC_write(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    if (IIC_write(st.hw->addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        IIC_write(st.hw->addr, 0x23, 1, &tmp);
        st.chip_cfg.dmp_on = 1;
...
static int setup_compass(void)
{
    ...
    if (IIC_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
//会在后面的mpu_get_compass_reg里将data * st.chip_cfg.mag_sens_adj右移8位，相当于除以了256，完成了磁力计灵敏度的调整
    ...
    if (IIC_write(st.hw->addr, st.reg->s0_addr, 1, data))
        return -1;

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (IIC_write(st.hw->addr, st.reg->s0_reg, 1, data))
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (IIC_write(st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (IIC_write(st.hw->addr, st.reg->s1_addr, 1, data))
        return -1;

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (IIC_write(st.hw->addr, st.reg->s1_reg, 1, data))
        return -1;

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (IIC_write(st.hw->addr, st.reg->s1_ctrl, 1, data))
        return -1;
    ...
    data[0] = 0x03;
    if (IIC_write(st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))
        return -1;

#ifdef MPU9150
    /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
    data[0] = BIT_I2C_MST_VDDIO;
    if (IIC_write(st.hw->addr, st.reg->yg_offs_tc, 1, data))
        return -1;
#endif
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(unsigned short *mag, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
    if (!(st.chip_cfg.sensors & INV_XYZ_COMPASS))
    ...
    if (IIC_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (IIC_write(st.hw->addr, st.reg->motion_thr, 2, data))
            goto lp_int_restore;
        ...
        if (IIC_write(st.hw->addr, st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (IIC_write(st.hw->addr, st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (IIC_write(st.hw->addr, st.reg->lp_accel_odr, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (MPU6500 version). */
        data[0] = BITS_WOM_EN;
        if (IIC_write(st.hw->addr, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
