...
namespace Probes
{
    ...
    public partial class TimeMeasurementBTControl : MeasurementBaseBTControl
    {
        ...
        public bool IsDisplaying => this.DisplayCheckBox.IsChecked.HasValue && this.DisplayCheckBox.IsChecked.Value;
        ...
        {
            if (this.ComPort!=null && this.ComPort.IsOpen && !string.IsNullOrEmpty(command))
            {
                try
                {
        ...
        public void Input(double t2,double t1)
        {
            if (!this.IsPausing)
            {
                double dt = t2 - t1;

                var ct = (DateTime.Now - this.StartTime).TotalSeconds;
                
                this.SyncPlot(ct, dt * this.ScaleFactor);
            }
        }
